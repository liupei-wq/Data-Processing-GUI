...
def _force_descending_x(ax) -> None:
...
def fit_plot(fit_result: dict, sample: str):
    ...
    axes[0].plot(fit_result["energy"], fit_result["observed"], color="#111827", lw=1.7, label=f"{sample} 觀測值")
    axes[0].plot(fit_result["energy"], fit_result["fit"], color="#E42213", lw=1.6, label="pDOS 擬合")
