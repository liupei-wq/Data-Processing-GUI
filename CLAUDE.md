[2026-05-20] 修正：Single Process Tool 高斯模板扣除三項 bug fix 並改善 UX。① 匯出 CSV 改用 client-side 即時運算值（`clientGaussianModel` / `clientAfterY`）而非後端快照（`dataset.y_gaussian_model`），修正「匯出數據沒扣到高斯」問題；② `clientAfterY` 改依 `gaussianNonnegativeGuard` flag 決定是否 clamp ≥0，修正非負保護被忽略的 bug；③ 移除右上角單一「匯出 CSV」按鈕，改在圖 1（原始+高斯模型）與圖 2（扣除後）各自底部加「↓ 匯出」按鈕，點擊先跳出 Origin Pro 風格預覽 modal，確認後才下載；④ 「切到最低點」改為需手動啟用才顯示，啟用後自動偵測資料全域範圍作為初始搜尋區間。影響檔案：`web/frontend/src/pages/SingleProcessTool.tsx`；驗證 `npm run build` 通過。

[2026-05-20] 修正：XAS 移除「高斯模板扣除」步驟（sidebar Section 6 + 主畫面圖卡），步驟重新編號（White Line 7→5、峰擬合 8→6）；歸一化圖卡底部加入 TEY 與 TFY 獨立匯出按鈕（依 normView 自動選 normalized/flattened 資料，疊圖模式輸出所有 overlay 資料）；清理相關殘留程式：移除 `GaussPeak` 未使用 import、刪除 `getChannelAfterGaussian` 函式（已無用）並將 `buildOverlayBackgroundComparisonTraces` 改為直接 fallback、移除 `hasGaussianStage` 始終為 false 的條件（`needsPreNormalizationRequest` 邏輯簡化）。影響檔案：`web/frontend/src/pages/XAS.tsx`；驗證 `npm run build` 通過（無 TypeScript 錯誤）。

[2026-05-21] 改善：XPS VBM 匯出 modal 兩項調整。① 切線/基準線 TXT 輸出範圍改為各自獨立短範圍：切線只覆蓋 `[min(edgeLo, vbm.x)-0.4, max(edgeHi, vbm.x)+0.4]`、基準線只覆蓋 `[min(baselineLo, vbm.x)-0.4, max(baselineHi, vbm.x)+0.4]`，兩條線各自延伸至 VBM 交點即停，不再使用共用的大範圍；② 預覽圖改為 Origin Pro 風格（白底、黑光譜/紅切線/藍基準線、黑色框線與刻度、legend 白底框）；參數卡新增「TXT 輸出範圍」標示各自短範圍。影響檔案：`web/frontend/src/pages/XPS.tsx`；驗證 `npm run build` 通過。

[2026-05-21] 修正：XPS VBM 匯出按鈕改為直接嵌在「VBM 線性外推圖」卡片底部，不再依賴 `currentDisplayDataset`（在「匯入已處理光譜」模式下 `currentDisplayDataset` 為 null，導致整個匯出區隱藏）。現在切線與基準線計算完成後（`vbmPreviewTangent && vbmPreviewBaselineLine`），圖卡底部即出現帶有預覽 VBM 值的 bar，以及「↓ 匯出 TXT（Origin Pro）」按鈕；點擊仍觸發 `showVbmExportPreview` modal。影響檔案：`web/frontend/src/pages/XPS.tsx`；驗證 `npm run build` 通過。

[2026-05-21] 新增：XPS Valence Band VBM 匯出按鈕加入「匯出預覽 modal」。點擊「VBM 外推光譜 TXT（Origin Pro）」後先跳出 modal，內含 VBM 圖表（切線/基準線/VBM 交點/區間陰影）、切線與基準線參數卡（區間/斜率/截距）、VBM 結果卡（預覽值與後端確認值）、以及 TXT 檔案前幾行預覽；底部「取消」關閉 modal，「確定匯出」才執行下載。新增 `showVbmExportPreview` state；modal 使用 z-50 fixed overlay，以 IIFE 從 closure 取得所有計算值避免重複 hook。影響檔案：`web/frontend/src/pages/XPS.tsx`；驗證 `npm run build` 通過。

[2026-05-21] 新增：XAS 新增「Conduction Band」分析模式（與 XPS Core Level / Valence Band 切換同一概念）。sidebar 頂部新增兩個模式切換 pill（XAS 分析 / Conduction Band）；Conduction Band 模式下隱藏所有既有 XAS 步驟，顯示兩個 CBM 專屬步驟：① 步驟 1「資料來源 / 歸一化」：可切換「處理流程結果（TEY / TFY）」或「匯入已處理光譜（TXT / CSV）」，資料載入後自動套用 Min-Max 歸一化（y → (y-min)/(max-min)）；② 步驟 2「CBM 線性外推」：與 XPS VBM 完全相同的算法（`fitEdgeLine` 切線取最大正斜率、基準線取最平斜率，`intersectEdgeLines` 求交點 = CBM）；主畫面顯示 CBM 外推圖，含橘色切線區間陰影、紫色基準線區間陰影、綠色 CBM 菱形標記；圖下方有雙把手拉桿可調整兩個區間，底部有「預覽 CBM = X.XXX eV」與「↓ 匯出 TXT（Origin Pro）」按鈕；匯出預覽 modal 包含 Origin Pro 風格圖（白底黑光譜紅切線藍基準線）、切線/基準線參數卡、預覽 CBM 值與 TXT 前幾行預覽；點「確定匯出」下載 `xas_cbm_*.txt`，欄位為 `Energy_eV | Spectrum_normalized | Tangent_Line | Baseline`，各線只在自己的範圍 ±0.4 eV 內填值，超出填 NaN，末行為 CBM 交點；新增 helper 函式 `fitEdgeLine` / `intersectEdgeLines` / `buildCbmStablePlotWindow`。影響檔案：`web/frontend/src/pages/XAS.tsx`；驗證 `npm run build` 通過（無 TypeScript 錯誤）。

[2026-05-21] 新增：XPS Valence Band 匯出區新增「VBM 外推光譜 TXT（Origin Pro）」按鈕。不需後端確認即可匯出；條件為 `effectiveVbmDataset && vbmPreviewTangent && vbmPreviewBaselineLine`；輸出 tab-separated TXT，含 `#` 注釋表頭（匯出時間/樣品名/切線區間/基準線區間/斜率/截距/預覽 VBM/後端確認 VBM），以及 `Binding_Energy_eV | Spectrum | Tangent_Line | Baseline` 四欄數據；`Tangent_Line` 與 `Baseline` 只在合併區間 ± margin 內填值，超出範圍填 `NaN`；最末附加 VBM 交點單行（`Spectrum=NaN`，兩條線各自外推值）；檔名依樣品名自動命名。影響檔案：`web/frontend/src/pages/XPS.tsx`；驗證 `npm run build` 通過。

[2026-05-21] 新增：XPS Valence Band / VBM 線性外推支援「匯入已處理光譜」資料來源。在 `web/frontend/src/pages/XPS.tsx` 新增 `parseTwoColumnText` helper（自動跳過 # 注釋行、自動偵測分隔符號、sort by x）、三個新 state（`vbmDataSource`、`importedVbmDataset`、`importedVbmError`）與 `effectiveVbmDataset` / `effectiveVbmBeMin` / `effectiveVbmBeMax`；VBM Section 頂部新增「處理流程結果 / 匯入已處理光譜」切換 pill，匯入模式下顯示檔案選擇器、解析摘要（名稱/點數/能量範圍）與移除按鈕；`computeVbmFn`、所有 useMemo（`vbmGlobalExtrema`、`vbmEdgeExtrema`、`vbmPreviewTangent`、`vbmPreviewBaselineLine`、`vbmPlotWindow`）、chart card 條件、chart data、`renderRangeControlCard` min/max、`disabled` 條件全部改用 `effectiveVbmDataset`；支援直接讀入外部 XES CSV（例如 `xes_1019(45-5).csv`、`xes_1008(40-10).csv`、`xes_1018(50-0).csv`）進行 VBM 線性外推分析。驗證 `npm run build` 通過。
...
[2026-05-20] 除錯：修正「單一處理工具 → 高斯模板扣除」兩個 bug。① 匯出 CSV 的 gaussian_model、gaussian_subtracted、processed 欄位使用的是後端舊快照（只在上傳檔案或按「重新載入」時更新），導致拖動 slider 調整參數後圖表已更新但 CSV 完全沒反映；修正：buildSingleToolCsv 新增 gaussianModelOverride/gaussianSubtractedOverride 參數，handleExport 在 gaussian 模式下傳入目前的 clientGaussianModel/clientAfterY 覆蓋後端快照值，processed 欄位也同步使用前端即時結果。② 避免負值保護開關被 clientAfterY 完全忽略，任何情況都強制 Math.max(0, ...)；修正：clientAfterY 依 gaussianNonnegativeGuard 決定是否截斷。影響檔案：web/frontend/src/pages/SingleProcessTool.tsx；驗證 npm run build 通過。
