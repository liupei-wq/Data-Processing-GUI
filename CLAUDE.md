...
- 2026-05-11 CST：開始修改 Raman fitting 程式，需求包含樣品相關峰資料庫限制、Si 520 cm^-1 獨立處理/遮罩、分區擬合、物理 FWHM/中心限制、不可分辨峰標記、先 baseline 再 fitting，以及報告中 520 cm^-1 標示為 Si dominated。

- 2026-05-11 CST：實作：開始調整 Raman 峰資料庫白名單與可選相預設關閉策略。

- 2026-05-11 CST：實作：開始改寫 Raman /api/raman/fit 為 Si 區獨立處理與材料分區擬合流程。

- 2026-05-11 CST：檢查：準備對 Raman 後端資料庫與 router 執行 py_compile。

- 2026-05-11 CST：檢查：系統 python.exe 無法啟動，改用專案 .venv Python 執行 Raman 後端 py_compile。

- 2026-05-11 CST：檢查：專案根目錄無 .venv，改用 uv run python 對 Raman 後端執行 py_compile。

- 2026-05-11 CST：檢查：uv cache 在沙盒內存取被拒，改請求非沙盒執行 Raman 後端 py_compile。

- 2026-05-11 CST：實作：開始更新 Raman 前端型別、API payload 與 fitting UI 控制項。

- 2026-05-11 CST：檢查：準備以 uv run 執行 Raman segmented fitting 合成光譜 smoke test。

- 2026-05-11 CST：檢查：前端 npm run build 嘗試時在 web/frontend 子目錄讀 CLAUDE.md 路徑不對；實際環境亦無 npm，無法在此工具環境完成前端 build。

- 2026-05-11 CST：檢查：前後端修改後再次執行 Raman 後端 py_compile。

- 2026-05-11 CST：檢查：執行 git diff --check 檢查 whitespace。

- 2026-05-11 CST：實作：移除誤產生的 web/frontend/CLAUDE.md，保留專案根目錄 CLAUDE.md 為唯一紀錄檔。

- 2026-05-11 CST：檢查：清理未使用 import 後再次執行 Raman 後端 py_compile。

- 2026-05-11 CST：完成 Raman fitting 修改：資料庫 fitting 白名單限制為 Si substrate、β-Ga2O3、NiO 與預設關閉 optional α-Ga2O3/GaOOH/Ni(OH)2；/api/raman/fit 改為先 baseline、Si 520 獨立 mask 或 fit/subtract、分區擬合、物理中心/FWHM 限制、unresolved 峰距檢查與 Si dominated 報告；前端新增 fitting baseline 與 Si 處理控制。驗證 py_compile 與 git diff --check 通過；數值 smoke test 因 uv root 環境缺 numpy 未完成，npm run build 因工具環境無 node/npm 未執行。
