...
- `web/frontend/src/pages/XPS.tsx`
  - 移除舊文案：
    - `每一筆資料都會沿用自己左欄保存的內插、背景扣除、歸一化等設定`
  - 改成：
    - `多筆疊圖模式會使用獨立的一套內插、背景扣除與歸一化參數，不會沿用單筆資料處理時的設定`
- 原因：
  - 舊文案是前一版邏輯留下來的，和現在已拆開的單筆 / 疊圖雙模式不一致

---

## XPS 圖表 bug 修正（2026-04-29, 續）

### 第二張圖標題改成依實際步驟顯示
- `web/frontend/src/pages/XPS.tsx`
  - 新增 `getStageDisplayLabel()`
  - 依目前實際啟用的前處理步驟組標題：
    - 只做內插 → `內插後疊圖`
    - 內插 + 平均 → `內插 / 平均後疊圖`
    - 只做校正 → `校正後疊圖`
...
### 歸一化文案
- `mean_region` 顯示名稱從 `算術平均` 改成 `Mean Region`
