...
- 2026-04-26 重新閱讀 `CLAUDE.md` 後，修正右欄自動捲動黃光未出現、捲動位置偏高的問題。根本原因：Codex 加入了 `_emit_scroll_script` 的 flash 動畫 CSS/JS，但所有圖表的 `auto_scroll_on_appear` 呼叫都沒有傳 `flash=True`（預設為 False），導致 `flashTarget()` 函式一開頭就 return。同時 `block="center"` 讓圖表只捲到 viewport 中央，使用者看不到圖表上半部。修正方式：1. 強化 `core/ui_helpers.py` 的 flash 動畫（outline 從 2px 增加到 4px，`rgba` 透明度提高到 1，持續 2.2s）；2. 改進 `flashTarget` 的目標元素搜尋，加入 `offsetHeight < 30` 跳過邏輯以忽略零高度中繼容器；3. 在 `modules/raman.py` / `modules/xps.py` / `modules/xrd.py` 所有圖表 `auto_scroll_on_appear(visible=True)` 呼叫加上 `flash=True, block="start"`，讓圖表頂端靠近 viewport 頂部，顯示大部分圖表高度。驗證完成：`python3 -m py_compile core/ui_helpers.py modules/raman.py modules/xps.py modules/xrd.py app.py` ✅、`git diff --check` ✅。
