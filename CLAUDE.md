...
- 2026-05-02 00:55 CST：讀取 `CLAUDE.md` 並重新檢查「所有模組上傳後都黑畫面」的可能來源；比對 `269e3a6..4a807d4` 後確認 `v18.5` 一次改動了 `App.tsx`、`WorkspaceUi.tsx`、`PlotPopupHost/PlotPopup`、`SpectrumChart`、`GaussianSubtractionChart` 與各模組頁面，屬於跨模組共用前端大改版，因此目前較高機率是朋友同步推上 GitHub 的共用改動造成 runtime 問題，而不是僅由本地 XPS 單頁修改引起。
- 2026-05-02 01:13 CST：讀取 `CLAUDE.md` 後沿著共用圖表層排查「選擇檔案並確認後全模組黑畫面」問題；發現所有模組在上傳後都會共同走進 `web/frontend/src/components/plotConfig.ts` 的 `withPlotFullscreen()`，因此先暫時移除最近新增的 Plotly 自訂全螢幕 modebar 按鈕，只保留安全的基本圖表 config，避免自訂按鈕在圖表初次 render 時造成跨模組 runtime 異常。
- 2026-05-02 01:15 CST：執行 `python3 -m py_compile web/backend/main.py web/backend/routers/*.py web/backend/core/peak_fitting.py` 與 `cd web/frontend && npm run build` 驗證 `plotConfig.ts` 的共用修正；後端語法檢查與前端建置皆通過，僅保留既有 Vite chunk size 警告。
- 2026-05-02 01:22 CST：讀取 `CLAUDE.md` 後在 `web/frontend/src/main.tsx` 新增全域 `RootErrorBoundary`，讓前端在 runtime 發生例外時不再整頁黑掉，而是直接把錯誤訊息顯示在畫面中央，方便進一步定位是哪個共用元件或模組在上傳後爆掉。
- 2026-05-02 01:24 CST：執行 `python3 -m py_compile web/backend/main.py web/backend/routers/*.py web/backend/core/peak_fitting.py` 與 `cd web/frontend && npm run build` 驗證 `RootErrorBoundary` 調整；後端語法檢查與前端建置皆通過，僅保留既有 Vite chunk size 警告。
- 2026-05-02 01:28 CST：根據使用者補充「只有網頁版黑、本地不會」，重新縮小問題範圍為部署差異而非本地邏輯錯誤；檢查後確認目前仍有本地未推送修改 `web/frontend/src/main.tsx`，因此現在線上站極可能仍在跑較舊的 bundle 或快取版本，下一步應優先考慮 hard refresh、推送本地修正並重新部署，而不是繼續只在本地邏輯層盲查。
- 2026-05-02 01:31 CST：依使用者同意準備推送本地修正，確認目前待提交內容為 `web/frontend/src/main.tsx` 與 `CLAUDE.md`；下一步將直接 commit 並 push 到 GitHub，讓 Render 重新部署線上站，驗證黑畫面是否來自部署版本落後。
- 2026-05-02 01:33 CST：將 `web/frontend/src/main.tsx` 與 `CLAUDE.md` 提交為 `Add frontend runtime error boundary`，並成功 `git push` 到 `origin/main`（`300db92 -> a5edf2f`）；現在線上網站可開始重新部署到含全域錯誤邊界的新版本。
- 2026-05-02 01:42 CST：根據使用者提供的 `Minified React error #130`，重新判斷黑畫面根因更接近「component type invalid」而非單純物件 render；結合「所有模組都在上傳後第一次 render Plotly 圖表時一起壞掉」的症狀，推測 `react-plotly.js` 在升級後的 production bundle 匯入相容性有問題。
- 2026-05-02 01:45 CST：新增 `web/frontend/src/components/PlotlyChart.tsx` 作為 `react-plotly.js` 兼容層，統一將 default export 與 namespace-style export 正規化後再提供給各頁面與共用圖表元件使用；同步把 `XRD / Raman / XAS / XES / XPS / SingleProcessTool / SpectrumChart / GaussianSubtractionChart` 的 `Plot` 匯入改成走這個兼容層，以避免 production 環境把 `Plot` 當成 object 而觸發 React #130。
- 2026-05-02 01:47 CST：執行 `python3 -m py_compile web/backend/main.py web/backend/routers/*.py web/backend/core/peak_fitting.py` 與 `cd web/frontend && npm run build` 驗證 Plotly 兼容層調整；後端語法檢查與前端建置皆通過，僅保留既有 Vite chunk size 警告。
...
- 2026-05-03 CST：讀取 `CLAUDE.md`，在 XRD 弱峰檢視步驟卡新增「匯出弱峰轉換圖譜 .txt」出口；修改 `web/frontend/src/components/ProcessingPanel.tsx`（Props 新增 `onExportWeakPeakSeries? / canExportWeakPeak?`，步驟卡底部加匯出按鈕）與 `web/frontend/src/pages/XRD.tsx`（傳入 `handleExportTransformedWeakPeakSeriesTxt` 與 `weakPeakTransformedSeries.x.length > 0`）；有資料時按鈕啟用，無資料時 disabled 灰顯。
