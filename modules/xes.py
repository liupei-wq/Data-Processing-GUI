...
def _parse_xes_spectrum_bytes(raw: bytes) -> tuple[np.ndarray | None, np.ndarray | None, str | None]:
...
def run_xes_ui():
    ...
    def _parse_optional_xes_spectrum(uploaded_spectrum, label: str):
        ...
        if cache_key not in st.session_state:
            x_vals, y_vals, err = _parse_xes_spectrum_bytes(uploaded_spectrum.getvalue())
    ...
    if using_preprocessed_spectra:
        ...
        for uf in spectrum_files:
            ...
            if cache_key not in st.session_state:
                x_vals, y_vals, err = _parse_xes_spectrum_bytes(uf.getvalue())
                st.session_state[cache_key] = (x_vals, y_vals, err)
