...
def run_xrd_ui() -> None:
    ...
    with st.sidebar:
        ...
        with st.expander(step_exp_label(2, "內插化及平均化", s2 or _skip2), expanded=not (s2 or _skip2)):
    ...
    with st.sidebar:
        ...
        if step2_done:
            ...
            with st.expander(step_exp_label(3, "高斯模板扣除", s3_gauss or _skip3_gauss), expanded=not (s3_gauss or _skip3_gauss)):
            ...
            skip_gauss = False
        step3_done = step2_done and (skip_gauss or s3_gauss)
    ...
    with st.sidebar:
        ...
        if step3_done:
            ...
            with st.expander(step_exp_label(4, "平滑", s4 or _skip4), expanded=not (s4 or _skip4)):
            ...
            skip_smooth = False
        step4_done = step3_done and (skip_smooth or s4)
    ...
    with st.sidebar:
        ...
        if step4_done:
            ...
            with st.expander(step_exp_label(5, "歸一化", s5 or _skip5), expanded=not (s5 or _skip5)):
            ...
            skip_norm = False
        step5_done = step4_done and (skip_norm or s5)
    ...
    with st.sidebar:
        ...
        if step5_done:
            ...
            with st.expander(step_exp_label(6, "取對數（弱峰檢視）", s6 or _skip6), expanded=not (s6 or _skip6)):
            ...
            skip_log = False
        step6_done = step5_done and (skip_log or s6)
