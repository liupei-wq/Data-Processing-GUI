...
- 2026-05-11：Raman fitting 改為材料白名單與分區物理限制流程：峰資料庫對 fitting 只暴露 Si substrate、β-Ga₂O₃、NiO，以及預設關閉的 α-Ga₂O₃ / GaOOH / Ni(OH)₂；GaN、TiO₂ 等樣品無關峰不再進預設 fitting library。`/api/raman/fit` 先做平滑 baseline，再獨立處理 Si 520 cm⁻¹（預設 mask 505–535，也可 fit/subtract），接著分 100–500、580–700、850–1150、1300–1700 cm⁻¹ 擬合，套用中心/FWHM 限制、空區不硬擬合、不可分辨峰標示 unresolved 且不參與定量；報告與 peak table 明確標示 520 cm⁻¹ 為 Si substrate dominated。前端新增 fitting baseline、Si 處理模式與 mask/main region 控制。驗證 `uv run python -m py_compile web\backend\routers\raman.py web\backend\db\raman_database.py`、`git diff --check` 通過；數值 smoke test 因 root `uv run` 缺 `numpy` 未完成，前端 `npm run build` 因工具環境無 node/npm 未執行。
